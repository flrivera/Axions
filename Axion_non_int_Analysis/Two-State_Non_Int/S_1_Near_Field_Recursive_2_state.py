...
#from sympy import *
#z, s11, s10,v02,gamma_c,mu = symbols('z s11 s10 v02 gc mu')
#s0 ,r= symbols('s0,r')
...
def Coeffiecients_1(gamma_c,mu,N):
    S_new=np.zeros((N+1),dtype=object)
    #V=np.zeros((N+1),dtype=object)
    ...
    for n in range(0, N-1):
        ...
        S_new[n+2]=-1*sum((v[m]*S_new[n-m]) for m in range (0,n+1)) + mu*m_l*S_new[n] #+ 3*gamma_c*sum(sum((s[o]*s[m-o]*S[n-m]) for o in range(0,m+1))for m in range(0,n+1))
            #print(s[n+2])
        S_new[n+2]=(S_new[n+2])*coeff
       

    return S_new

def phi_small_1(r,mu,gamma_c,nlimit):

  
    sumS =0.0
    sumV= 0.0
    for n in range(0,nlimit+1):
        s_n=Coeffiecients_1(gamma_c,mu,nlimit)
        sumS += s_n[n] * r**n
