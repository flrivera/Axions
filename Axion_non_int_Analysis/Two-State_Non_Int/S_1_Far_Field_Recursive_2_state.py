...
def Coeffiecientslarge(gamma_c,alpha,beta,N,M):
...
def phi_large(rad,gamma_c,nlimit,mlimit):
    ...
    for n in range(0,nlimit+1):
        for m in range (0,mlimit+1):
            ...
            s_n,v_n=Coeffiecientslarge(gamma_c,alpha,beta,nlimit,mlimit)
...
def Coeffiecientslarge1(S1_10,gamma_c,mu,sigma,alpha,beta,N,M):
    
    s,v= Coeffiecientslarge(gamma_c,alpha,beta,N,M)
    m_l=1
    l=1
  ...
  #  sigma=1-beta
    ...
    for n in range(0, N+1):
        ...
        if n==0:
            ...
            for m in range (1,M+1):
                coeff1= 1/(2*beta + 2*np.sqrt(1+mu)*(sigma + m -1))

                coeff2= np.sqrt(1+mu)*(l*(l+1)-(sigma +m-1)*(sigma+m-2))


                S_new[n,m]= (S_new[n,m-1]*coeff2)*coeff1
           ...
           # coeff_s=((1+mu)*n**2 - mu)
            ...
            #for m in range (0,M+1):
              ...
              #  if m==0:
                    ...
                    #sums=  -1*sum((v[p,m]*S_new[n-p,m]) for p in range (0,1) )
               ...
               # elif m==1:
                 ...
                 #   coeff1= (1+mu)**(-m/2)*(n**2 * (1+mu)**(2/2)+ v[0,0] -mu)
                    
                 #   coeff3= (1+mu)**(-m/2)*(2*n*(1+mu)**(2/2)*(sigma*n+m-2) + v[0,1]*(1+mu)**(1/2))
                   # sums=sum(sum((v[p,q]*S_new[n-p,m-q]) for p in range (0,n+1) )for q in range (0,m+1))
                    #gamma_term=3* gamma_c* (sum(sum(sum(sum(s[r,t]*s[p-r,q-t]*S_new[n-p,m-q] for t in range (0,q+1))for r in range (0,p+1))for p in range (0,n+1))for q in range (0,m+1)))
                        
                
                #    S_new[n,m]=(-coeff3*S_new[n,m-1])*(1/coeff1)
                    #print(s[n,m])
                        #sumv=sum(sum(s[p,q]*S_new[n-p,m-q] for p in range (0,n+1) )for q in range (0,m+1))
                    #print(s[n+2])
                    
                        #V_new[n,m]=(-2*sumv-coeff3*V_new[n,m-1])*coeff2
                
             #   elif m>=2:

                    #print('m=',m)
                 #   coeff1s= (1+mu)**(-m/2)*(n**2 * (1+mu)**(2/2)+ v[0,0] -mu)
                 #   coeff1= (1+mu)**(-m/2)*(2*n*(1+mu)**(2/2)*(sigma*n+m-2) + v[0,1]*(1+mu)**(1/2))
                 #   coeff2= (1+mu)**(-m/2)*(1+mu)**(2/2)*((sigma*n +m-2)*(sigma*n +m-3)-l*(l+1))
                   # sums=sum(sum((v[p,q]*S_new[n-p,m-q]) for p in range (0,n+1) )for q in range (0,m+1))
                    #gamma_term= 3*gamma_c* (sum(sum(sum(sum(s[r,t]*s[p-r,q-t]*S_new[n-p,m-q] for t in range (0,q+1))for r in range (0,p+1))for p in range (0,n+1))for q in range (0,m+1)))
                        
                        #sumv=sum(sum(s[p,q]*S_new[n-p,m-q] for p in range (0,n+1) )for q in range (0,m+1))
                        
                   # S_new[n,m]=(-coeff1*S_new[n,m-1]- coeff2*S_new[n,m-2])*(1/coeff1s)
                        #V_new[n,m]=(-2*sumv-coeff1*V_new[n,m-1]- coeff2*V_new[n,m-2])/(n**2)

                

    return  S_new

def phi_large_1(rad,S1_10,gamma_c,mu,nlimit,mlimit):
    #S0=.01
    #mu=.01
    
    alpha=3.4951309897
    beta= 1.7526648513
    sigma=1- beta/(np.sqrt(1+mu))
  ...
  #  sumV= 0.0
    for n in range(0,nlimit+1):
        for m in range (0,mlimit+1):
            ...
            s_n=Coeffiecientslarge1(S1_10,gamma_c,mu,sigma,alpha,beta,nlimit,mlimit)
            s_n=s_n.astype(float)
            sumS += s_n[n,m] * (np.exp(-np.sqrt(1+mu)*rad)/(np.sqrt(1+mu)*rad)**sigma)**n * (np.sqrt(1+mu)*rad)**-m
           # sumV += v_n[n,m] * (np.exp(-rad)/rad**sigma)**n * rad**-m
    return sumS 

def phi_large_1n(rad,S1_10,gamma_c,mu,nlimit,mlimit):
    #S0=.01
    #mu=.01
    
    alpha=3.4951309897
    beta= 1.7526648513
    sigma=1- beta/(sym.sqrt(1+mu))
  ...
  #  sumV= 0.0
   ...
   #         
            s_n=Coeffiecientslarge1(S1_10,gamma_c,mu,sigma,alpha,beta,nlimit,mlimit)
            sumS += s_n[n,m] * (sym.exp(-1*sym.sqrt(1+mu)*rad)/(sym.sqrt(1+mu)*rad)**sigma)**n * (sym.sqrt(1+mu)*rad)**-m
    #       # sumV += v_n[n,m] * (np.exp(-rad)/rad**sigma)**n * rad**-m
    return sumS 